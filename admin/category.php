...
if (isset($_POST['action']) && $_POST['action'] == 'addCategory') {
    ...
    $name = $connection->escape_string(trim($_POST['txtCategoryName']));
    ...
    if (empty($name)) {
        $has_error = true;
        $error_message["txtCategoryName"] = "Please Enter Category Name";
    }
    ...
    if ($has_error) {
        ...
        $args = [
            "categoryName" => $name,
            "categoryStatus" => STATE_SUCCESS,
            "isActive" => ACTIVE
        ];
        $insert_id = insert_query($connection, "app_category", $args);

        if ($insert_id) {
            $success_message = "Category Successfully Added";
            header("Refresh:0");
        } else {
            $error_message["form"] = "Unable to Add Category At the Moment. Please Try again Later";
        }
    }
}
        ...
        <div class="col form-portlet">
            ...
            <form method="post" action="#" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <input type="text"
                           class="form-control  <?php echo isset($error_message['txtCategoryName']) ? 'is-invalid' : '' ?>"
                           id="txtCategoryName" name="txtCategoryName">
                    <div class="invalid-feedback">
                        <?php echo isset($error_message['txtCategoryName']) ? $error_message['txtCategoryName'] : '' ?>
                    </div>
                </div>
    ...
    <div class="row mt-5">
        <div class="col">
            <table class="table table-light">
                ...
                if ($category_result && $category_result->num_rows) {
                    $count = 0;
