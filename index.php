...
$book_count = $book_result->fetch_array()[0];
...
$question_count = $question_result->fetch_array()[0];
...
$project_count = $project_result->fetch_array()[0];

$category_query = sprintf("SELECT COUNT(*) FROM app_category WHERE isActive='%s' AND categoryStatus='%s'",ACTIVE,STATE_SUCCESS);
$category_result = $connection->query($category_query);

$category_count = $category_result->fetch_array()[0];

$user_query = sprintf("SELECT COUNT(*) FROM app_user WHERE isActive='%s' AND userStatus='%s'",ACTIVE,STATE_SUCCESS);
$user_result = $connection->query($user_query);

$user_count = $user_result->fetch_array()[0];

$course_query = sprintf("SELECT COUNT(*) FROM app_course WHERE isActive='%s' AND courseStatus='%s'",ACTIVE,STATE_SUCCESS);
$course_result = $connection->query($course_query);

$course_count = $course_result->fetch_array()[0];

$department_query = sprintf("SELECT COUNT(*) FROM app_department WHERE isActive='%s' AND departmentStatus='%s'",ACTIVE,STATE_SUCCESS);
$department_result = $connection->query($department_query);

$department_count = $department_result->fetch_array()[0];

$faculty_query = sprintf("SELECT COUNT(*) FROM app_faculty WHERE isActive='%s' AND facultyStatus='%s'",ACTIVE,STATE_SUCCESS);
$faculty_result = $connection->query($faculty_query);

$faculty_count = $faculty_result->fetch_array()[0];
