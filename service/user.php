...
if (isset($_GET['action'])) {
    ...
    switch ($_GET['action']) {
        ...
        case 'update':
            ...
            //in this part we will handle the registration
            if (is_these_parameters_available(array('user_id','email', 'first_name', 'last_name', 'department', 'matric_no', 'level'), $input)) {
                ...
                //checking if the user is already exist with this email
                //as the email should be unique for every user
                $check_query = sprintf("SELECT * FROM app_user WHERE userEmail='%s' AND userID='%s'", $email, $user_id);
                $check_result = $connection->query($check_query);
                ...
                if ($check_result->num_rows > 0) {
                    ...
                    if(isset($password)){
                        $args['userPass'] = $password;
                    }
                    $insert_id = update_query($connection, "app_user", $args,[
                        "userID" => $user_id
                    ]);
                    ...
                    if ($insert_id) {
                        ...
                        //fetching the user back
                        $user_query = sprintf("SELECT * FROM app_user WHERE isActive='%s' AND userID='%s'", ACTIVE, $user_id);
                        $user_result = $connection->query($user_query);
                        $user = $user_result->fetch_assoc();
        ...
        case 'profile':
            ...
            //in this part we will handle the registration
            if (is_these_parameters_available(array('user_id'), $input)) {
